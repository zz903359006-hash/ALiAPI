...
export default function UsageAnalyticsPage() {
  ...
  const handleExport = () => {
    const csv = [
      ["时间", "API Key", "模型", "输入 Token", "输出 Token", "结果状态"].join(","),
      ...filtered.map((r) => [r.time, r.keyId, r.model, r.inputTokens, r.outputTokens, r.status].join(",")),
    ].join("\n");
  ...
  return (
    <div>
      ...
      {/* KPI cards — 4 token-focused metrics */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "var(--spacing-lg)", marginBottom: "var(--spacing-lg)" }}>
        <KpiCard label="总 Token 消耗" value={totalTokens >= 1000000 ? `${(totalTokens / 1000000).toFixed(2)}M` : `${(totalTokens / 1000).toFixed(0)}K`} sub={`${totalRequests} 次请求`} />
        <KpiCard label="输入 Token 消耗" value={totalInput >= 1000000 ? `${(totalInput / 1000000).toFixed(2)}M` : `${(totalInput / 1000).toFixed(0)}K`} sub={`${((totalInput / totalTokens) * 100 || 0).toFixed(0)}% 占比`} />
        <KpiCard label="输出 Token 消耗" value={totalOutput >= 1000000 ? `${(totalOutput / 1000000).toFixed(2)}M` : `${(totalOutput / 1000).toFixed(0)}K`} sub={`${((totalOutput / totalTokens) * 100 || 0).toFixed(0)}% 占比`} />
        <KpiCard label="总请求次数" value={`${totalRequests}`} sub="所选时间区间" />
      </div>
          ...
          <table style={{ width: "100%", minWidth: 800, borderCollapse: "collapse", marginTop: "var(--spacing-sm)" }}>
            <thead>
              <tr style={{ backgroundColor: "#F9FAFB" }}>
                <Th>时间</Th><Th>API Key</Th><Th>模型</Th><Th right>输入 Token</Th><Th right>输出 Token</Th><Th>结果状态</Th>
              </tr>
            </thead>
            <tbody>
      ...
      <div style={{ padding: "var(--spacing-lg)", display: "flex", flexDirection: "column", gap: 10 }}>
        {data.map((d, i) => (
          <div key={d.name} style={{ display: "flex", alignItems: "center", gap: "var(--spacing-sm)" }}>
            ...
            <div style={{ flex: 1, height: 20, backgroundColor: "var(--color-surface-card)", borderRadius: "var(--radius-xs)", overflow: "hidden" }}>
              <div style={{ width: `${(d.pct / maxPct) * 100}%`, height: "100%", backgroundColor: "var(--color-primary)", borderRadius: "var(--radius-xs)", minWidth: 4, transition: "width 0.3s" }} />
            </div>
